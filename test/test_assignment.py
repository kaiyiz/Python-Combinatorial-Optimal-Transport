...
# load the test data
    ...
    M, yA, yB, total_cost = assignment_torch(cost_tensor, C_tensor, delta_tensor, device=device)
    assert np.allclose(M_res, M.numpy().astype(int), atol=1e-5)
    assert np.allclose(total_cost_res, total_cost.numpy(), atol=1e-5)

def test_assignment_numpy_precmoputed():
    cost = np.loadtxt('./test/test_data/assignment_cost_test.csv', delimiter=',')
    M_res = np.loadtxt('./test/test_data/assignment_M_res_test_numpy.csv', delimiter=',')
    total_cost_res = np.loadtxt('./test/test_data/assignment_total_cost_test_numpy.csv', delimiter=',')
    delta = np.loadtxt('./test/test_data/assignment_delta_test_numpy.csv', delimiter=',')
    C = cost.max()
    M, yA, yB, total_cost = assignment(cost, C, delta)
    assert np.allclose(M_res, M, atol=1e-5)
    assert np.allclose(total_cost_res, total_cost, atol=1e-5)